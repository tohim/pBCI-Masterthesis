...
base_path = fullfile('E:', 'SchuleJobAusbildung', 'HTW', 'MasterThesis', ...
                     'Code', 'Matlab', 'Data', 'AutoCalibration', version_tag, '1000samples_30Pct_Calib');

filename = fullfile(base_path, 'AfterCalib_1000Total_210Calib_Results.xlsx');
...
fprintf('\nEvaluation of CLASS WISE Metrics POST Calibration, Version %s: \n', version_tag);
fprintf('\n');
...
for i = 1:length(datasets)
    ...
    T.CalibrationType = lower(strtrim(string(T.CalibrationType)));
    T.Class = strtrim(string(T.Class));
    T.Source = strtrim(string(T.Source));
    T.Target = strtrim(string(T.Target));

    % Add Config based on whether Source or Target contains "25wCsp"
    ...
    % Filter: HYPER model, finetuned, and 25wCsp config
    isValid = strcmp(T.Model, 'HYPER') & strcmp(T.CalibrationType, 'finetuned') & strcmp(T.Config, '25wCsp');
    T = T(isValid, :);

    % Exclude within-target
    T(strcmp(T.Target, 'CalibrationSource (WITHIN)'), :) = [];
